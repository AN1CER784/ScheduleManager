...
class GenerateSummaryView(JsonFormMixin,CommonFormMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        form = GenerationForm(data={'username': request.user.username, 'start_date': start_date, 'end_date': end_date, 'period': period}, request=request)
        if form.is_valid():
    ...
    def form_valid(self, form):
        form.save()
        return JsonResponse(self.response(message='Your summary will be generated soon', item_html=None, success=True))

    def form_invalid(self, form):
        item_html = self.render_errors(form=form, request=self.request)
        return JsonResponse(self.response(message='Your summary could not be generated', item_html=item_html, success=False))
...
class DeleteSummaryView(CommonFormMixin, View):
    def post(self, request, *args, **kwargs):
        ...
        report.delete()
        return JsonResponse(self.response(message='Summary was deleted', item_html=None, success=True))
