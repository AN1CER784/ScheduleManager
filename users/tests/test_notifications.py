...
class UserSignUpNotificationTestCase(TransactionTestCase):
    ...
    def test_user_signup_notification(self, mock_delay):
        ...
        User.objects.create_user(username='testuser', email='test@example.com', password='123456', company=company)
        self.assertTrue(mock_delay.called)

    def test_user_signup_notification_builder(self):
        company = Company.objects.create(name="Acme")
        user = User.objects.create_user(username='testuser', email='test@example.com', password='123456', company=company)
...
class UserResetPasswordNotificationTestCase(TransactionTestCase):
    ...
    def test_user_reset_password_notification(self, mock_notify):
        company = Company.objects.create(name="Acme")
        user = User.objects.create_user(username='XXXXXXXX', email='test@example.com', password='123456', company=company)
